...
import {
    ...
    FETCH_DATA_ERROR,
    FETCH_DATA_LIST_GROUP,
    FETCH_DATA_LOADING,
    FETCH_DATA_TIPE_AKUN,
    FORM_DATA,
    FORM_DELETE
...
export const fetchDataSuccess = (data) => {
    return {
        type: FETCH_DATA_TIPE_AKUN,
        payload: data
    }
}

export const fetchDataListGroups = (data) => {
    return {
        type: FETCH_DATA_LIST_GROUP,
        payload: data
    }
}
...
export const fetchData = (param) => {
    ...
    return async (dispatch) => {
        ...
        return await axios.get(API_URL + "/back-office/data-tipe-akun" + param, {
            headers: {
...
export const fetchDataDetail = (param) => {
    ...
    const dt_res = dt.toString(CryptoJS.enc.Utf8);
    const _dt = dt_res.split('Þ');
    return axios.get(API_URL + "/back-office/edit-data-tipe-akun/" + param, {
        headers: {
            'x-app-origin': 'backoffice-app',
            'Authorization': 'Bearer ' + _dt[0]
        }
...
export const addData = (param) => {
    ...
    return async (dispatch) => {
        ...
        const _data = {};
        await axios.post(API_URL + "/back-office/action-data-tipe-akun", param, {
            headers: {
                'x-app-origin': 'backoffice-app',
                'Authorization': 'Bearer ' + _dt[0]
            }
            ...
            .then(response => {
                ...
                if (err_code === 0) {
                    ...
                    _data['contentMsg'] = <div
                        dangerouslySetInnerHTML={{__html: '<div style="font-size:20px; text-align:center;"><strong>Failed</strong>, Silahkan lengkapi data terlebih dahulu</div>'}}/>;
                    dispatch(addDataSuccess(_data));
                    dispatch(fetchAddDataLoading(isLoading));
                }
...
export const deleteData = (param) => {
    ...
    return async (dispatch) => {
        dispatch(fetchAddDataLoading(isLoading));
        const _data = {};
        await axios.get(API_URL + "/back-office/delete-data-tipe-akun/" + param, {
            headers: {
                'x-app-origin': 'backoffice-app',
                'Authorization': 'Bearer ' + _dt[0]
            }
...
export const fetchDataListGroup = (param) => {
    ...
    return async (dispatch) => {
        ...
        return await axios.get(API_URL + "/api/general-option/list-group", {
            headers: {
                'x-app-origin': 'backoffice-app',
                'Authorization': 'Bearer ' + _dt[0]
            }
            ...
            .then(response => {
                ...
                if (response.data.error_message === 0) {
                    data['demo'] = response.data.payload.demo
                    data['real'] = response.data.payload.real
                    dispatch(fetchDataListGroups(data));

                } else {
                    const errorpayload = {};
                    errorpayload['message'] = 'Something wrong';
                    errorpayload['status'] = response.data.error_message;
                    console.log('fetchDataListGroup');
