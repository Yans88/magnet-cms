...
import {
    ...
    fetchDataLoading,
    showConfirmDel,
} from "./dataKotaService";
import {fetchData as fetchDataProvinsi} from "../data-provinsi/dataProvinsiService";
...
class DataKota extends Component {
    constructor(props) {
        super(props);
        this.initSelected = {
            kota_id: "",
    ...
    editRecord = async (record) => {
        let isLoading = true;
        let dataDetail;
        await fetchDataDetail(record.kota_id)
            .then((response) => {
    ...
    handleSubmit = async () => {
        ...
        errors.nama_kota = !this.state.selected.nama_kota
            ? "Nama Kota required"
            : "";
        errors.provinsi_id = !this.state.selected.provinsi_id
            ? "Provinsi required"
            : "";
        this.setState({errors, provinsi: this.state.selected.provinsi_id});
        if (this.validateForm(this.state.errMsg)) {
            let selected = {
                ...this.state.selected,
                provinsi: this.state.selected.provinsi_id,
            };
    ...
    handleDelete = async () => {
        await this.props.onDelete(this.state.selected.kota_id);
        this.props.onLoad(
            "?search" +
            this.state.keyword +
            "&limit=" +
    ...
    render() {
        const {data, dataProvinsi, allowedMenu} = this.props;
        ...
        if (allowedMenu && (allowedMenu.kota_edit || allowedMenu.kota_delete)) {
            columns = [
                ...columns,
                {
                    key: "action",
                    ...
                    cell: (record) => {
                        return (
                            <div style={{textAlign: "center"}}>
                                <Fragment>
                                    {allowedMenu && allowedMenu.kota_edit ? (
                                        <button
                                    ...
                                    {allowedMenu && allowedMenu.kota_delete ? (
                                        <button
        ...
        const frmUser = (
            <Form id="myForm">
                <Form.Group controlId="nama_kota">
                        ...
                        {dataProvinsi.map((item, index) => {
                            return (
                                <option
                                    ...
                                    selected={item.provinsi_id === selected.provinsi_id}
                                >
                                    {item.nama_provinsi}
                                </option>
                            );
                        ...
                        <div className="container-fluid">
                            <div className="row">
                                <div className="col-12">
                                    ...
                                    >
                                        {allowedMenu && allowedMenu.kota_add ? (
...
const mapStateToProps = (state) => {
    return {
        data: state.dataKota.data || [],
        isLoading: state.dataKota.isLoading,
        isAddLoading: state.dataKota.isAddLoading,
        error: state.dataKota.error || null,
        errorPriority: state.dataKota.errorPriority || null,
        totalData: state.dataKota.totalData || 0,
        showFormAdd: state.dataKota.showFormAdd,
        contentMsg: state.dataKota.contentMsg,
        showFormSuccess: state.dataKota.showFormSuccess,
        showFormDelete: state.dataKota.showFormDelete,
        tipeSWAL: state.dataKota.tipeSWAL,
        dataProvinsi: state.dataProvinsi.data || [],
...
export default connect(mapStateToProps, mapDispatchToPros)(DataKota);
