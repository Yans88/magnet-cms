...
export const fetchData = (param) => {
    ...
    return async (dispatch) => {
        ...
        return await axios.get(API_URL + "/back-office/get-list-approve" + param, {
            headers: {
            ...
            .then(response => {
                ...
                if (response.data.error_message === 0) {
                    ...
                    for (var i = 0; i < response.data.payload.data.length; i++) {
                        let key = {...response.data.payload.data[i], _key: nanoid(30)};
                        res.push(key);
                    }
...
export const showConfirmAppr = (dt) => {
    const data = {};
    data['errorPriority'] = null;
...
export const rejectData = (param) => {
    ...
    return async (dispatch) => {
        dispatch(fetchAddDataLoading(isLoading));
        const _data = {};
        await axios.post(API_URL + "/back-office/reject-data-user/" + param, {
            headers: {
                'x-app-origin': 'backoffice-app',
                'Authorization': 'Bearer ' + _dt[0]
            }
...
export const apprData = (param) => {
    ...
    return async (dispatch) => {
        dispatch(fetchAddDataLoading(isLoading));
        const _data = {};
        await axios.get(API_URL + "/back-office/approve-data-user/" + param, {
            headers: {
                'x-app-origin': 'backoffice-app',
                'Authorization': 'Bearer ' + _dt[0]
            }
            ...
            .then(response => {
                dispatch(showConfirmAppr(false));
                const err_code = response.data.error_message;
                if (err_code === 0) {
                    isLoading = false;
                    _data['showFormSuccess'] = true;
